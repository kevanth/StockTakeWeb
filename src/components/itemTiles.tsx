...
export default function ItemTile({
  ...
  const debouncedUpdate = useRef(
    debounce(
      async (
        ...
        try {
          await updateItem(
            new Item(item.id, name, count, category, description)
          );
        } catch (err) {
          const message =
  ...
  const buttonClass =
    "border-1 border-border hover:bg-border aspect-square w-8";
  ...
  return (
    <div className="flex flex-col items-center border text-foreground border-border p-4 rounded bg-card aspect-square">
      <div className="flex flex-row w-full ">
        ...
        <button
          className={
            "h-8 w-8 ml-1 hover:text-red-400 hover:bg-red-500/10 flex items-center justify-center"
          }
          onClick={() => {
            deleteItem(item.id);
            refreshItems();
      ...
      <div className=" relative w-full">
        <button
          className={
            "border-2 my-1 rounded-full px-2.5 py-1 text-sm max-w-full truncate mb-4 " +
